...
class HistoricalReaderNewFormat:
    def __init__(self, path_new, path_old, step_in_hours, asset_file_history):
        ...
        #     [datetime.datetime.strptime(f[-12 - len('_hourly'):-4 - len('_hourly')], '%Y%m%d') for f in file_names_new])
        ...
        self.mult = np.array(np.append(np.array(np.zeros_like(sorted_file_names_old, dtype=int)),
        ...
        self.today = []
        self.fake_today = []
        self.step = step_in_hours
        self.asset_file_history = asset_file_history
        self.strikes_count = []
    ...
    def get_data_from_file(self, call_put_diff, N1, N2, read_row='all', step=1):
        ...
        df_spot = pd.read_csv(f'C:\\Users\\admin\\for python\\HISTORY\\spot\\{self.asset_file_history}', header=None,
                              names=['datetime', 'spot'])
        try:
            df_spot['datetime'] = [datetime.datetime.strptime(d, '%Y-%m-%d %H:%M:%S') for d in
                                   df_spot['datetime'].values]
        except ValueError:
            df_spot['datetime'] = [datetime.datetime.strptime(d, '%Y-%b-%d %H:%M:%S') for d in
                                   df_spot['datetime'].values]
        ...
        if read_row == 'first':
            ...
            needed_datetime_list = np.arange(min_date,
                                             max_date,
                                             datetime.timedelta(hours=self.step)).astype(datetime.datetime)
        ...
        for zz, k in enumerate(step_range):
            ...
            if len(self.today) > 2 and self.today[-2] != self.today[-1]:
                self.spot.append(float(df_small.u_price.iloc[0]))
            else:
                try:
                    self.spot.append(float(df_spot[df_spot['datetime'] == needed_datetime_list[zz]]['spot'].iloc[0]))
                except:
